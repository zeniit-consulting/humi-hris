...
export default function PortalApprovalsPage({
    ...
    const act = async (item: Item, action: 'approve' | 'reject') => {
        ...
        const reason =
            action === 'reject' ? window.prompt('Alasan penolakan') : null;
            ...
            <section className="rounded-[16px] bg-white px-5 py-5 shadow-[0_16px_42px_rgba(15,23,42,0.07)]">
                <div className="flex items-center gap-3">
                    <span className="portal-primary-soft inline-flex size-11 items-center justify-center rounded-lg">
                        <ClipboardCheck className="portal-primary-text size-5" />
                    </span>
                    <div>
                        ...
                        items.map((item) => (
                            ...
                            >
                                <p className="text-sm font-semibold">
                                    {labels[item.type]} · {item.employee_label}
                                </p>
                                <p className="mt-1 text-sm text-slate-500">
                                    Tahap approval {item.stage} · Diajukan{' '}
                                    {item.created_at ?? '-'}
                                </p>
                                <div className="mt-3 flex gap-2">
                                    <button
                                        ...
                                        className="inline-flex items-center gap-1 rounded-md bg-emerald-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-50"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="inline-flex items-center gap-1 rounded-md bg-rose-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-50"
                                    >
                                        <X className="size-4" />
                                        Tolak
                                    </button>
