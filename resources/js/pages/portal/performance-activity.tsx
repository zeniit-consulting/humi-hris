...
export default function PortalPerformanceActivityPage({ pageTitle }: Props) {
    ...
    const loadReviews = async () => {
        const response = await requestApi<PerformancePayload>(
            '/portal/api/performances',
        );
        setReviews(response.data.items);
        setSelectedReviewId(
            (current) => current ?? response.data.items[0]?.id ?? null,
        );
    };
