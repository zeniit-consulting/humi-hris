...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Approval Absensi', href: pageUrl }];
...
export default function AttendanceApprovalPage() {
    const { requests, employees, filters, statusOptions, stats } = usePage<PageProps>().props;
    const [filterState, setFilterState] = useState(filters);
    const [detailRow, setDetailRow] = useState<AttendanceRequestRow | null>(null);
    const [rejectRow, setRejectRow] = useState<AttendanceRequestRow | null>(null);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-4 p-4">
                ...
                <Card>
                    <CardHeader><CardTitle>Filter Approval Absensi</CardTitle></CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Total data: {requests.total}</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="overflow-x-auto">
                            <table className="w-full min-w-[980px] text-sm">
                                ...
                                <tbody>
                                    {requests.data.length === 0 && <tr><td colSpan={6} className="px-3 py-6 text-center text-muted-foreground">Belum ada request absensi.</td></tr>}
                                    {requests.data.map((row) => (
                                        <tr key={row.id} className="border-b align-top">
                                            ...
                                            <td className="px-3 py-3">{row.attendance_date}<div className="text-xs text-muted-foreground">{formatShift(row.shift)}</div></td>
                                            <td className="px-3 py-3">Masuk {formatTime(row.check_in_at)}<div className="text-xs text-muted-foreground">Pulang {formatTime(row.check_out_at)}</div></td>
            ...
            <Dialog open={detailRow !== null} onOpenChange={(open) => !open && setDetailRow(null)}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Detail Request Absensi</DialogTitle><DialogDescription>{detailRow?.employee_label}</DialogDescription></DialogHeader>
                    {detailRow && <div className="space-y-3 text-sm">
                        <p><strong>Tanggal:</strong> {detailRow.attendance_date}</p>
                        <p><strong>Shift:</strong> {formatShift(detailRow.shift)}</p>
                        <p><strong>Jam:</strong> Masuk {formatTime(detailRow.check_in_at)} · Pulang {formatTime(detailRow.check_out_at)}</p>
            ...
            <Dialog open={rejectRow !== null} onOpenChange={(open) => !open && setRejectRow(null)}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Tolak Request Absensi</DialogTitle><DialogDescription>Masukkan alasan penolakan.</DialogDescription></DialogHeader>
